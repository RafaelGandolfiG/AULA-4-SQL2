...
CREATE TABLE clientes(
	...
	--datetime 2 gera a data e hora com precisao
	--a generated: essa coluna é gerada autom,aticamente pelo sistema
	...
	--definir o periodo em tempo durante o qual o registro é valido
	PERIOD FOR SYSTEM_TIME (data_inicio, data_fim)
)
--ativando o versionamento do sistema e criando uma tabela historico
WITH(SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.clientes_historico));


--cria a tabela historico que armazenará as versoes anteriores dos dados,
--por padrao o sql cria essa tabela automaticamente quando o versionamento
--é habilitado, mas podemos criar explicitamente se desejado
...
--essas tabelas sao uteis para armazenar dados 
--temporarios que nao precisam persistir no banco de dados
